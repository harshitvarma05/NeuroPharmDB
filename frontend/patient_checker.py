...
def show_patient_checker():
    ...
    st.markdown(
        ...
        <div class='glass-card'>
            ...
            <p style='margin:6px 0 0 0' class='muted'>
                Select two drugs. The system checks known interactions and can also generate an AI-based suggestion that requires doctor review.
            </p>
        </div>
        """,
    ...
    # -------------------------
    # Choose doctor to send AI suggestion to
    # -------------------------
    ...
    if doctors:
        ...
        for d in doctors:
            did = d.get("user_id")
            nm = d.get("name") or "Doctor"
            em = d.get("email") or ""
            if did:
                label = f"{nm} ({did})" if not em else f"{nm} ({did}) • {em}"
                label_to_id[label] = did

        if label_to_id:
            pick = st.selectbox(
                "Send AI suggestion to doctor",
                options=list(label_to_id.keys()),
                key="pc_pick_doctor",
            )
            selected_doctor_id = label_to_id.get(pick)
        else:
            st.info("No doctor/admin users found. Create a doctor/admin user first.")
    else:
        st.info("No doctor/admin users found. Create a doctor/admin user first.")
    ...
    if not known:
        ...
        sev = float(known.get("severity_score", 0.0) or 0.0)
        ...
        # Auto-alert patient if high risk (scoped to this user)
        if user_id and sev >= 7.0:
            try:
                ...
                # Duplicate/constraint errors are ok to ignore here for demo stability
                pass
    ...
    st.caption(
        "AI suggestions are generated by a simple demo heuristic and may be incorrect. "
    ...
    if st.button("Generate AI suggestion", use_container_width=True, key="pc_gen_ai"):
        try:
            ...
            sid = dbc.store_ai_suggestion(
                ...
                target_doctor_id=selected_doctor_id,  # IMPORTANT: sends only to chosen doctor
            )
